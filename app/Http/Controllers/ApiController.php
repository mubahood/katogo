...
class ApiController extends BaseController
{
    ...
    public function me_update(Request $r)
    {
        ...
        if ($phone !== null) {
            ...
            if (!method_exists($mergeService, 'autoMergeMatchesIntoCurrentUser')) {
                Log::warning('ME_UPDATE_MERGE_METHOD_MISSING', [
                    'user_id' => $u->id,
                    'phone' => $phone,
                ]);
    ...
    public function profile_wizard_contact(Request $r)
    {
        ...
        if (!$skipAutoMerge) {
            $mergeService = app(AccountMergeService::class);
            if (!method_exists($mergeService, 'autoMergeMatchesIntoCurrentUser')) {
                Log::warning('PROFILE_WIZARD_CONTACT_MERGE_METHOD_MISSING', [
                    'target_user_id' => $u->id,
